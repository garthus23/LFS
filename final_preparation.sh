...
./configure --prefix=/usr \
	--docdir=/usr/share/doc/bison-3.7.5  > /dev/null 2>> $ERROR
make > /dev/null 2>> $ERROR
make install > /dev/null 2>> $ERROR
if [[ -f /usr/bin/bison ]]
then
	...
	echo -e "Bison not installed [${RED}FAILED${WHITE}]"
	exit 2
fi
cd /sources
rm -rf bison-3.7.5
...
sh Configure -des \
	...
	-Dvendorarch=/usr/lib/perl5/5.32/vendor_perl > /dev/null 2>> $ERROR
make > /dev/null 2>> $ERROR
make install > /dev/null 2>> $ERROR
if [[ -f /usr/bin/perl ]]
then
	...
	echo -e "Perl not installed [${RED}FAILED${WHITE}]"
	exit 2
fi
cd /sources
rm -rf perl-5.32.1
...
./configure --prefix=/usr \
	--enable-shared \
	--without-ensurepip > /dev/null 2>> $ERROR
make > /dev/null 2>> $ERROR
make install > /dev/null 2>> $ERROR
...
then
	...
	echo -e "Python not installed [${RED}FAILED${WHITE}]"
	exit 2
fi
cd /sources
rm -rf Python-3.9.2
...
then
	...
	echo -e "Texinfo not installed [${RED}FAILED${WHITE}]"
	exit 2
fi
cd /sources
rm -rf texinfo-6.7
...
then
	...
	echo -e "Util-Linux not installed [${RED}FAILED${WHITE}]"
fi
cd /sources
